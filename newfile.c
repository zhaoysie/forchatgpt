 ...
 */
>>>>>>> 9ec9dca64e02481a72a332f53187dcc751ad9efb
								...
								sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "%s: pass event %d: %03d %s\n", nua_event_name(event),
					  event, status, phrase);
...
void *SWITCH_THREAD_FUNC sofia_profile_thread_run(switch_thread_t *thread, void *obj)
{
...
#ifdef ZHZS_ENABLE_PRECONDITION
	int use_precondition = !sofia_test_pflag(profile, PFLAG_DISABLE_PRECONDITION);
#endif
	...
	supported = switch_core_sprintf(profile->pool, "%s%s%spath, replaces", 
		use_100rel ? (use_precondition ? "precondition, 100rel, " : "100rel, ") : "",
		use_timer ? "timer, " : "", use_rfc_5626 ? "outbound, " : "");
#else
...
#ifdef ZHZS_ENABLE_PRECONDITION
					sofia_set_pflag(profile, PFLAG_DISABLE_PRECONDITION);
#endif
...
#ifdef ZHZS_ENABLE_PRECONDITION
					} else if (!strcasecmp(var, "enable-precondition")) {
						if (switch_true(val)) {
							sofia_clear_pflag(profile, PFLAG_DISABLE_PRECONDITION);
						}
						else {
							sofia_set_pflag(profile, PFLAG_DISABLE_PRECONDITION);
						}
									  ...
									  tagi_t tags[])
{
	...
	if (sip && session) {
		...
		switch_core_session_t *other_session;
		private_object_t *tech_pvt = switch_core_session_get_private(session);
        private_object_t *othertech_pvt = NULL;
		char network_ip[80];
		int network_port = 0;
		...
		//{ change by zhoujc makesure the invite rsp throught to other leg
		if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
			othertech_pvt = switch_core_session_get_private(other_session);
		}
		if (status > 199 &&
			(switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA) ||
			 (switch_channel_test_flag(tech_pvt->channel, CF_T38_PASSTHRU) && (has_t38 || status > 299)) ||
			 (NULL != othertech_pvt && sofia_test_flag(othertech_pvt, TFLAG_REINVITED)))) {
			if (sofia_test_flag(tech_pvt, TFLAG_SENT_UPDATE)) {
				...
				if ((uuid = switch_channel_get_partner_uuid(channel)) && (other_session = switch_core_session_locate(uuid))) {
					...
					msg = switch_core_session_alloc(other_session, sizeof(*msg));
					if ( (NULL != othertech_pvt && sofia_test_flag(othertech_pvt, TFLAG_REINVITED))) {
						msg->message_id = SWITCH_MESSAGE_INDICATE_REINVITE_RESPONSE;
					}else{
						msg->message_id = SWITCH_MESSAGE_INDICATE_RESPOND;
					}					
					msg->from = __FILE__;
									 ...
									 tagi_t tags[])
{
  ...
  state_process:
	...
	case nua_callstate_received:
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "hand reinvite at received state\n");
		...
		if (!sofia_test_flag(tech_pvt, TFLAG_SDP)) {
			...
			if (r_sdp && !sofia_test_flag(tech_pvt, TFLAG_SDP)) {
				...
				if (sofia_test_pflag(profile, PFLAG_3PCC)) {
					if (switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA)) {
						...
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
										  "Channel %s  state received respond 200OK by PFLAG_3PCC\n",
			...
			if (zstr(tech_pvt->mparams.local_sdp_str)) {
				...
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
								  "Channel %s  state received respond 200OK by r_sdp is null\n",
								  switch_channel_get_name(channel));
				if (sofia_use_soa(tech_pvt)) {
					nua_respond(tech_pvt->nh, SIP_200_OK,
								SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
							  "Channel %s  state received change to completed\n", switch_channel_get_name(channel));
			goto state_process;
		}
	...
	case nua_callstate_early:
		...
		break;
	case nua_callstate_completed:
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "hand at received state\n");
		if (r_sdp) {
			const char *var;
			...
			tech_pvt->mparams.hold_laps = 0;
				...
				if (switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA)) {
					...
					if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
						...
						if (switch_channel_test_flag(channel, CF_PROXY_MODE) && !is_t38 &&
							...
							switch_log_printf(
								SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
								"Channel %s  state completed respond 200OK by CF_PROXY_MODE or media_on_hold(%d)\n",
								switch_channel_get_name(channel), media_on_hold);
								if (sofia_use_soa(tech_pvt)) {
									nua_respond(tech_pvt->nh, SIP_200_OK,
												SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
												SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str),
					...
					if (!is_t38 && hold_related && switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
						if (switch_core_session_compare(session, other_session)) {
							...
							if (sofia_test_pflag(profile, PFLAG_PROXY_HOLD)) {
								...
								switch_core_media_gen_local_sdp(session, SDP_TYPE_RESPONSE, NULL, 0, NULL, 0);
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
											  "Channel %s  state completed respond 200OK by hold_related(%d)\n",
											  switch_channel_get_name(channel), hold_related);
								if (sofia_use_soa(tech_pvt)){
									nua_respond(tech_pvt->nh, SIP_200_OK,
												SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
												SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str),
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
									  "Channel %s  state completed respond 200OK by CF_APP_T38_NEGOTIATED\n",
									  switch_channel_get_name(channel));
						if (sofia_use_soa(tech_pvt)) {
							nua_respond(tech_pvt->nh, SIP_200_OK,
										SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
				...
				//// add by zhoujc(20210917) tel the other leg the media change with reinvite,as proxy media
				if (!switch_channel_test_flag(channel, CF_PROXY_MODE)){
					if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
                        private_object_t *other_tech_pvt = NULL;
                        switch_core_session_message_t *msg = NULL;
                        
						switch_core_media_clear_rtp_flag(other_session, SWITCH_MEDIA_TYPE_AUDIO,
						...
						sofia_set_flag(tech_pvt, TFLAG_REINVITED);
						// make the sdp can use the new
						//		switch_channel_clear_flag(other_tech_pvt->channel, CF_ANSWERED);
						//		switch_channel_clear_flag(tech_pvt->channel, CF_ANSWERED);
						// make sure the invite rsp can transfer to a leg
						//		switch_channel_set_callstate(other_tech_pvt->channel, CCS_EARLY);
						//	switch_channel_set_callstate(tech_pvt->channel, CCS_EARLY);
						...
						msg->string_arg = switch_core_session_strdup(other_session, r_sdp);
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
										  "reinvite Passing SDP to other leg.\n%s\n", r_sdp);
						...
						if (sofia_test_flag(tech_pvt, TFLAG_SIP_HOLD)) {
							...
							if (sofia_test_pflag(profile, PFLAG_MANAGE_SHARED_APPEARANCE)) {
								const char *info = switch_channel_get_variable(channel, "presence_call_info");
								if (info) {
									if (switch_stristr("private", info)) { msg = "hold-private"; }
								}
							}
						...
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
							"Channel %s  state completed switch_core_session_get_partner fail \n",switch_channel_get_name(channel));
					}
				}
					if (tech_pvt->mparams.num_codecs) {
						match = sofia_media_negotiate_sdp(session, r_sdp, SDP_TYPE_REQUEST);
					...
					if (match) {
						...
						if (switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA)) {
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
							"Channel %s  state completed respond 200OK by CF_PROXY_MEDIA or CF_PROXY_MODE\n",
							switch_channel_get_name(channel));
							nua_respond(tech_pvt->nh, SIP_200_OK, TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
							goto done;
						}
				...
				if (is_ok) {
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
									  "Channel %s  state completed respond 200OK by CF_AWAITING_STREAM_CHANGE\n",
									  switch_channel_get_name(channel));
						if (sofia_use_soa(tech_pvt)) {
							nua_respond(tech_pvt->nh, SIP_200_OK,
										SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
